...
public class ImportancePlace_PriceandPromotion_Style extends AppCompatActivity {
    ...
    public RecyclerView_Importance_Place_PricenPromotion_Adapter adapter;
    public int langcode;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_importance_place__priceand_promotion__style);
        setFaculty();
        ...
        String r = getResources().getString(R.string.near_place);
        this.bar.setTitle(r);
        ...
        this.getDistance(dormName);


    }

    public void getDistance(String dormName){
       final RequestQueue queue = Volley.newRequestQueue(this);
        String pathPositionMydorm = "http://"+ip+port+"/dorm/getDormByDormName/"+dormName;
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, pathPositionMydorm, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
                    double latitude = response.getDouble("latitude");
                    double longtitude = response.getDouble("longtitude");
                    int sizePlace = plist.size();
                    final Places[] p = new Places[sizePlace];
                    for(int count=0;count<sizePlace;count++)
                    {
                        final int c = count;
                        ...
                        String urlDirection = "https://maps.googleapis.com/maps/api/directions/json?"+"origin="+latitude+","+longtitude+"&destination="+lat+","+lng+"&key="+newServerKey;
                        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, urlDirection, null, new Response.Listener<JSONObject>() {
                            ...
                            public void onResponse(JSONObject response) {
                                ...
                                try
                                {
                                    ...
                                    adapter = new RecyclerView_Importance_Place_PricenPromotion_Adapter(getApplicationContext(),list,langcode);
