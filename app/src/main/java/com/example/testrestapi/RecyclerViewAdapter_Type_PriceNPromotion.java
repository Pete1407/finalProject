...
public class RecyclerViewAdapter_Type_PriceNPromotion extends RecyclerView.Adapter<RecyclerViewAdapter_Type_PriceNPromotion.RecyclerViewHolder> {
    ...
    public RecyclerViewAdapter_Type_PriceNPromotion(ArrayList<Dorm_PricenPromotion_Model> l,Context c){
        this.context = c;
        this.list = l;
    }
   ...
   final RecyclerViewHolder holder = hold;
   String unit = context.getResources().getString(R.string.unit);
   String was = context.getResources().getString(R.string.was);
   int c = context.getResources().getColor(android.R.color.black);
        if(list.get(position).getStatus_promotion() == 1)
        {
            ...
            holder.oldprice.setText(String.valueOf(list.get(position).getNewPrice())+" "+unit);
            holder.oldprice.setTypeface(Typeface.DEFAULT_BOLD);
            //holder.oldprice.setPaintFlags(holder.oldprice.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);

            //holder.nameDorm.setTextColor(c);
        ...
        {
            int red_dark = context.getResources().getColor(android.R.color.holo_red_dark);
            Picasso.with(context).load(list.get(position).getImage()).into(holder.image);
            holder.nameDorm.setText(list.get(position).getDorm_name());
            holder.nameDorm.setTextColor(red_dark);
            holder.distance.setText(list.get(position).getDistance()+" จาก "+list.get(position).getFaculty().getName_faculty());
            holder.tagsale.setVisibility(View.GONE);
            holder.oldprice.setText(String.valueOf(list.get(position).getOldPrice())+" "+unit);
            holder.oldprice.setTypeface(Typeface.DEFAULT_BOLD);
            holder.prodetail.setVisibility(View.GONE);
            holder.distance.setTextColor(c);
        }
